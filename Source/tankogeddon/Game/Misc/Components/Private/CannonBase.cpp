#include "tankogeddon/Game/Misc/Components/Public/CannonBase.h"
	...
	if (Type == ECannonType::FireProjectile){
		...
		if (GetWorld()->LineTraceSingleByChannel(HitResult, StartPoint, EndPoint, ECollisionChannel::ECC_Visibility, TraceSettings)){
			...
			if (HitResult.Actor.IsValid()){
				IDestructible* DamagedActor = Cast<IDestructible>(HitResult.Actor);
				...
				if(DamagedActor){
					...
					if (bIsDestroyed){
						...
						if(ScoredActor){
							...
							if(PlayerState)
								PlayerState->IncrementScore(1, HitResult.GetActor());
						}
						else{
							APlayerVehicle* Player = Cast<APlayerVehicle>(HitResult.GetActor());
							if(Player){
